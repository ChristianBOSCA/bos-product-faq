...
function evaluate(attachment, rack, claimText) {
  ...
  if (unknown(attachment.mount_points)) {
  ...
  if (unknown(attachment.needs_side_holes)) {
...
/* Dual-mode: require()d by the tests and the serverless function, and loaded
 * as a plain <script> by the browser so verdicts compute client-side with no
 * round trip — same approach the FAQ takes with its fuzzy search. */
const API = { evaluate, inferTubing, buildWorklist, unknown, VERDICT, DIMENSION_PRIORITY };
